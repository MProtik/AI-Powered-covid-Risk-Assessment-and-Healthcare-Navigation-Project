...
class Application:
    def __init__(self, root):
        ...
        self.data = []
        self.cursor = cursor  # Add cursor to the instance for use in methods
    ...
    def create_front_page(self):
        ...
        see_data_button = ttk.Button(self.root, text="Choose a Disease", command=self.choose_disease)
        see_data_button.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
    ...
    def choose_disease(self):
        root = tk.Tk()
        app = spinningwheel.SpinningWheelApp(root)
        root.mainloop()
    ...
    def print_user_data(self):
        ...
        user_data = self.user_info(search_name, search_phone)

        if not user_data:
            messagebox.showinfo("No Data", "No data found for the given name and phone number.")
            return

        self.display_user_data(user_data)

    def user_info(self, name, phone_number):
        query = 'SELECT * FROM user_database WHERE Name = %s AND `Phone Number` = %s;'
        self.cursor.execute(query, (name, phone_number))
    ...
    def display_user_data(self, user_data):
        ...
        # Create Treeview
        columns = ["Name", "Age", "Gender", "NID", "Address", "Upozila", "District", "Phone Number", "Email Address", "Posterior probability"]
        ...
        tree.pack(pady=15, expand=True, fill=tk.BOTH)
